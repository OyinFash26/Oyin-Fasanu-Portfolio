      ...
      <div className="grid gap-12 lg:grid-cols-[1.2fr_1fr] lg:gap-16">
        ...
        <Reveal delay={120} className="space-y-6">
          ...
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            <p className="eyebrow border-b border-border px-6 py-5">Profile Snapshot</p>
            <dl className="grid md:grid-cols-3">
              {PROFILE_SNAPSHOT.map((item, index) => (
                <Reveal
                  key={item.label}
                  delay={180 + index * 100}
                  className={`group border-border px-6 py-7 transition-colors duration-300 hover:bg-surface md:px-7 ${
                    index > 0 ? "border-t md:border-t-0 md:border-l" : ""
                  }`}
                >
                  <dt className="eyebrow transition-colors duration-300 group-hover:text-accent">{item.label}</dt>
                  <dd className="mt-4 font-display text-lg font-semibold leading-snug md:text-xl">{item.title}</dd>
                  {item.detail && (
